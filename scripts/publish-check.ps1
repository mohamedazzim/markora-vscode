...
if ([string]$package.publisher -ne 'MohamedAzzimJ') { Fail 'Marketplace publisher is not MohamedAzzimJ.' } else { Pass 'Publisher field is configured.' }
if ([string]$package.repository.url -ne 'https://github.com/mohamedazzim/markora-vscode.git') { Fail 'Repository metadata is not configured for mohamedazzim/markora-vscode.' } else { Pass 'Repository metadata is configured.' }
...
$oversized = Get-ChildItem -Recurse -File | Where-Object { $_.FullName -notmatch '\\node_modules\\|\\dist\\|\\release\\|\\.vscode-test\\|\\graphify-out\\|\\.codex\\|\\.git\\' -and $_.Length -gt 10MB }
